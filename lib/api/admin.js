...
export function registerAdminRoutes(r) {
  ...
  r.patch('/admin/settings', async (ctx) => {
    ...
    if (b.qr_settings && typeof b.qr_settings === 'object') {
      const list = (arr, max) => (Array.isArray(arr) ? [...new Set(arr.map((x) => String(x).trim().slice(0, 20)).filter(Boolean))].slice(0, max) : [])
      const cur = ctx.tenant.qr_settings || {}
      set.qr_settings = {
        tables: b.qr_settings.tables !== undefined ? list(b.qr_settings.tables, 500) : cur.tables || [],
        rooms: b.qr_settings.rooms !== undefined ? list(b.qr_settings.rooms, 1000) : cur.rooms || [],
        base_url: typeof b.qr_settings.base_url === 'string' ? b.qr_settings.base_url.trim().slice(0, 200) : cur.base_url || '',
      }
    }
